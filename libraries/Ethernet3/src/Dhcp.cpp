...
int DhcpClass::request_DHCP_lease(){
    ...
    while(_dhcp_state != STATE_DHCP_LEASED)
    {
        if(_dhcp_state == STATE_DHCP_START)
        {
            ...
            send_DHCP_MESSAGE(DHCP_DISCOVER, ((uint16_t)((millis() - startTime) / 1000)));
            _dhcp_state = STATE_DHCP_DISCOVER;
        }
        else if(_dhcp_state == STATE_DHCP_REREQUEST){
            _dhcpTransactionId++;
            send_DHCP_MESSAGE(DHCP_REQUEST, ((uint16_t)((millis() - startTime)/1000)));
...
// Because of the uncertainty of malloc on LLVM compller, DhcpClass instance has declared in advance
DhcpClass temp_Dhcp;
