...
extern "C" {
  ...
  extern volatile uint16_t g_csi20_tx_count;
...
#if     defined(CSI_CHANNEL4) && CSI_CHANNEL4 == 4
  case 4:
    Master_Create_          = R_Config_CSI20_Create;
    Master_Start_           = R_Config_CSI20_Start;
    Master_Stop_            = R_Config_CSI20_Stop;
    Master_Create_UserInit_ = R_Config_CSI20_Create_UserInit;
    Master_ClearStatus_     = R_Config_CSI20_ClearStatus;
    Master_Send_Receive_    = R_Config_CSI20_Send_Receive;
    Master_SetBitOrder_     = R_Config_CSI20_SetBitOrder;
    Master_SetDataMode_     = R_Config_CSI20_SetDataMode;
    Master_SetClockDivider_ = R_Config_CSI20_SetClockDivider;
    Master_SetClock_        = R_Config_CSI20_SetClock;
    Master_GetBitOrder_     = R_Config_CSI20_GetBitOrder;
    master_tx_count         = &g_csi20_tx_count;
    master_status_flag      = &g_csi20_status_flag;
    break;
#endif /* defined(CSI_CHANNEL4) && CSI_CHANNEL4 == 4 */
  default:
    Master_Create_          =
    Master_Start_           =
    Master_Stop_            =
...
uint16_t  CSI_Impl::Master_StatusFlagWait(void) const {
  ...
  uint16_t wait = 0;
  while (true) {
    uint16_t stat   = *master_status_flag;
    if((stat & 0x7F) == 0x10)
    {
      return 0;
    }
